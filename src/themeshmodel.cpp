...
void cModel::compute_curvatures() {
    ...
    m_curvatures_computed = true; // mark curvatures as calculated
}
...
void cModel::prepare_gaussian_curvature_preview() {
    ...
    for (auto& part : m_parts) {
        for (const auto& v : part->vertices) {
			if (v.second->curvature_info.gaussCurvature < -1.e-2) { // negative Gaussian curvature (saddle) in red
                v.second->curvature_map_value = 0;
            }
			else if (v.second->curvature_info.signGauss < 1.e-2) { // near zero Gaussian curvature (flat) in blue
                v.second->curvature_map_value = 1;
            }
            else
				v.second->curvature_map_value = 2;  // positive Gaussian curvature (sphere-like) in green
        }
    }
}
...
void cModel::prepare_k1_k2_preview() {
    ...
    for (auto& part : m_parts) {
        for (const auto& v : part->vertices) {
            double k1 = v.second->curvature_info.k_1;
            double k2 = v.second->curvature_info.k_2;
            ...
            if (k1 < -1.e-2) { // k_1 is negative
                if (k2 < -1.e-2) { // k_2 is negative, Concave ellipsoid 0 red
                    ...
                    v.second->curvature_map_value = 2;
                }
            }
                ...
                if (k2 < -1.e-2) { // k_2 is negative, Concave cylinder 1 blue
                    ...
                    v.second->curvature_map_value = 3;
                }
                else { // k_2 is positive, Convex cylinder 4 magenta
                    v.second->curvature_map_value = 4;
                }
            }
                ...
                if (k2 < -1.e-2) { // k_2 is negative, Hyperboloid 2 green
                    ...
                    v.second->curvature_map_value = 5;
                }
            }
        }
    }
...
void cModel::prepare_mean_curvature_preview() {
    ...
    for (auto& part : m_parts) {
        for (const auto& v : part->vertices) {
            double H = v.second->curvature_info.meanCurvature;
