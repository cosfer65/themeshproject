/// @file curvature.cpp
/// @brief Implementation of mesh curvature computation algorithms
/// 
/// This file implements algorithms for computing vertex curvatures on triangular meshes
/// using the second fundamental form. The implementation uses a Voronoi-weighted approach
/// to estimate principal curvatures and directions at mesh vertices.
...
Parameters
    ...
    - results
        ...
        On output, it contains:
          [0-5]   Projected normal differences in 2D (three edges × two components).
          [6-11]  Edge position differences in 2D (three edges × two components).
...
Notes
    ...
    - The rotation angle is computed from the arccos of the normalized dot product between
      `vertex_normal` and `face_normal`, with a sign convention given by the code (`-acos(...)`).
    - Rodrigues rotation formula is used to explicitly build the 3x3 rotation matrix R:
          R = I * cos(theta) + (1 - cos(theta)) * (k k^T) + [k]_x * sin(theta)
...
static void rotateElemPlane(dvec3& u_f, dvec3& v_f, dvec3& vertex_normal, dvec3& face_normal, basematrix<double, 2, 3>& result)
{
    ...
    // This is the standard Rodrigues rotation matrix for rotation around unit axis k = rotation_axis by angle θ :
...
/*
calculatePrincipalDirections computes principal curvature directions at a vertex:
...
static void calculate_vertex_curvatures(mesh<double>* fmesh, meshVertex<double>& v) 
{
    ...
    // Store the principal directions in the vertex curvature info  
    v.curvature_info.k_min_dir.normalize();
    v.curvature_info.k_max_dir.normalize();
}

/*
compute_vertex_curvatures is Main entry point for computing curvatures for all mesh vertices

prepares the mesh by computing face properties and vertex normals, then
iterates through all vertices to calculate their curvature information.
sets the mesh's curvatures_computed flag upon completion.

param: fmesh Pointer to the mesh to process (modified with curvature data)
*/
